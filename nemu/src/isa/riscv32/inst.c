...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */)                   \
  ...
  INSTPAT("???????????????????? ????? 11011 11", jal, J, R(rd) = s->pc + 4;
          s->dnpc = s->pc + imm; if (rd == 1) log_ftrace(s->dnpc, 1);
          else if (rd == 0 && src1 == 1) log_ftrace(s->dnpc, -1));
  INSTPAT("???????????? ????? 000 ????? 11001 11", jalr, I, R(rd) = s->pc + 4;
          s->dnpc = (src1 + imm) & ~1; if (rd == 1) log_ftrace(s->dnpc, 1);
          else if (rd == 0 && src1 == 1) log_ftrace(s->dnpc, -1));
  INSTPAT(
      "??????? ????? ????? 000 ????? 11000 11", beq, B,
